...
def test_vmap_regge_solver_scan_and_certificate():
    jnp = pytest.importorskip("jax.numpy")

    from src.regge_vmap_solver import VectorizedReggeSolver

    solver = VectorizedReggeSolver(N_t=16, t_min=1e-2, t_max=1e2)
    delta_mock = jnp.zeros(16) + 0.05

    traj = solver.scan_regge_trajectory(delta_mock)
...
def test_distributed_hessian_pl_callback_smoke():
    pl = pytest.importorskip("pytorch_lightning")
    torch = pytest.importorskip("torch")

    from src.callbacks.distributed_hessian_pl import DistributedHessianPLCallback
    ...
    model = DummyModule()
    cb = DistributedHessianPLCallback(monitor_every=1, warmup_steps=0, pl_tol=1e-6)
    trainer = pl.Trainer(
        max_epochs=1,
