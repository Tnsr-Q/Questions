...
def test_shard_map_regge_solver_scan_and_certificate():
    jnp = pytest.importorskip("jax.numpy")

    from src.regge_shard_map import ShardedReggeSolver

    solver = ShardedReggeSolver(N_t=16, t_min=1e-2, t_max=1e2)
    delta_mock = jnp.zeros(16) + 0.05

    traj = solver.scan_regge_trajectory_sharded(delta_mock)
...
def test_zero3_checkpointed_hessian_pl_callback_smoke():
    ...
    model = DummyModule()
    cb = Zero3CheckpointedHessianPLCallback(
        monitor_every=1,
        warmup_steps=0,
        pl_tol=1e-6,
